...
def args_parser():
    ...
    parser.add_argument('-dataset', type=str, default='Houston', # 默认数据集改为 IP
                        choices=['PaviaU', 'Houston', 'WHU'])  # 添加 IP 到可选列表
    ...
    parser.add_argument('--train_ratio', type=float, default=0.02,
...
def train(model, device, train_loader, optimizer, epoch, args):
    ...
    # 添加深度维度（假设depth=1）
    #inputs_1 = inputs_1.unsqueeze(1)  # 形状变为 [batch, channels, 1, H, W]
    optimizer.zero_grad()
