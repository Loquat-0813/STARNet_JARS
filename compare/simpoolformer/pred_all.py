...
def args_parser():
    ...
    parser.add_argument('--checkpointsmodelfile', type=str, default='/zbssd/yuyu/code/HyBenchmark/checkpoints/simpoolformer/Houston/model_83.20.pth')
    ...
    parser.add_argument('--allimg', type=bool, default=True, help='allimg')
            ...
            for j in range(width):
                if int(y[i, j]) == 0:
                    ...
                    image_patch = image_patch.reshape(1, image_patch.shape[0], image_patch.shape[1], image_patch.shape[2],
                                                    1)
                    X_test_image = torch.FloatTensor(image_patch.transpose(0, 4, 3, 1, 2)).to(device).squeeze(1)
