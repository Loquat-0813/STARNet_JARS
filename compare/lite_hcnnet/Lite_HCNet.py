...
class LE_HCL(nn.Module):#输入默认为一个三维块，即三维通道数为1
    ...
    def forward(self, x):
        ...
        return out
# 主模型
class Lite_HCNet(nn.Module):#定义了一个名为 Lite_HCNet 的类，它继承自 nn.Module，这是 PyTorch 中所有神经网络模块的基类。
...
if __name__ == '__main__':
    model = Lite_HCNet(15, 10, 9).cuda()#实例化 Lite_HCNet 模型，输入通道数为 15，分类类别数量为 10，块大小为 9，并将模型移动到 GPU 上。
    x = torch.randn(32, 15, 9, 9).cuda()#生成一个随机的输入张量，形状为 (32, 15, 9, 9)，表示批量大小为 32，通道数为 15，高度和宽度均为 9，并将其移动到 GPU 上。
    y = model(x)
